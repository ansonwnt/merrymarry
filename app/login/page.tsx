...
import { getUser, setUser, generateAccountNumber } from "../lib/storage";
...
export default function LoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email || !password) { setError("請填寫所有欄位 Please fill in all fields"); return; }
    const user = getUser();
    if (!user || user.email !== email) {
      setError("找不到帳戶 Account not found");
      return;
    }
    if (user.weddingDate) {
  ...
  function handleGoogle() {
    const user = getUser();
    if (user) {
      router.push(user.weddingDate ? "/dashboard" : "/profile");
    } else {
      const newUser = {
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#fff" }}>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-16">
        ...
        <p className="mt-6 text-xs" style={{ color: "#999" }}>
          還未有帳戶？{" "}
          <Link href="/register" style={{ color: "#4A2060" }}>
            立即註冊 Register
          </Link>
        </p>
      </main>
